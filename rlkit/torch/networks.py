...
class RecurrentEncoder(FlattenMlp):
    ...
    def __init__(self,
        ...
        self.lstm = nn.LSTM(self.hidden_dim, self.hidden_dim, num_layers=self.lstm_num_layers, batch_first=True)
    ...
    def forward(self, in_, return_preactivations=False):
        ...
        self.hidden = torch.zeros(self.lstm_num_layers, task, out.shape[-1]).to(ptu.device)
