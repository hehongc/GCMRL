...
class OfflineMetaRLAlgorithm(metaclass=abc.ABCMeta):
    ...
    def _try_to_eval(self, epoch):
        if self._can_evaluate():
            train_returns, avg_train_return, avg_test_return, \
            avg_test_online_all_return, avg_test_offline_all_return, avg_test_np_online_all_return, \
            avg_train_online_final_return, avg_train_offline_final_return, avg_train_np_online_final_return, \
            avg_train_online_all_return, avg_train_offline_all_return, avg_train_np_online_all_return, \
            avg_test_online_all_return_one_shot, avg_test_offline_all_return_one_shot, avg_test_np_online_all_return_one_shot, \
            avg_train_online_all_return_one_shot, avg_train_offline_all_return_one_shot, avg_train_np_online_all_return_one_shot = self.evaluate(epoch)
            ...
            return train_returns, avg_train_return, avg_test_return, \
                   avg_test_online_all_return, avg_test_offline_all_return, avg_test_np_online_all_return, \
                   avg_train_online_final_return, avg_train_offline_final_return, avg_train_np_online_final_return, \
                   avg_train_online_all_return, avg_train_offline_all_return, avg_train_np_online_all_return, \
                   avg_test_online_all_return_one_shot, avg_test_offline_all_return_one_shot, avg_test_np_online_all_return_one_shot, \
                   avg_train_online_all_return_one_shot, avg_train_offline_all_return_one_shot, avg_train_np_online_all_return_one_shot
        else:
    ...
    def _do_eval(self, indices, epoch, buffer):
        ...
        online_all_return_one_shot = [t[:1] for t in online_all_return]
        ...
        offline_all_return_one_shot = [t[:1] for t in offline_all_return]
        ...
        np_online_all_return_one_shot = [t[:1] for t in np_online_all_return]

        return online_final_returns, online_all_return, offline_final_returns, \
               offline_all_return, np_online_final_returns, np_online_all_return, \
               online_all_return_one_shot, offline_all_return_one_shot, np_online_all_return_one_shot
    ...
    def train(self):
        ...
        for it_ in gt.timed_for(range(self.num_iterations), save_itrs=True):
            ...
            for train_step in range(self.num_train_steps_per_itr):
                ...
                z_means, z_vars, wandb_stat = self._do_training(indices)
            ...
            if self.allow_eval:
                ...
                avg_train_online_all_return, avg_train_offline_all_return, avg_train_np_online_all_return, \
                avg_test_online_all_return_one_shot, avg_test_offline_all_return_one_shot, avg_test_np_online_all_return_one_shot, \
                avg_train_online_all_return_one_shot, avg_train_offline_all_return_one_shot, avg_train_np_online_all_return_one_shot = self._try_to_eval(it_)

                # wandb_stat["train_returns"] = np.mean(train_returns).item()
                # wandb_stat["avg_train_return"] = np.mean(avg_train_return).item()
                # wandb_stat["avg_test_return"] = np.mean(avg_test_return).item()
                #
                # wandb_stat["avg_test_online_all_return"] = np.mean(avg_test_online_all_return).item()
                # wandb_stat["avg_test_offline_all_return"] = np.mean(avg_test_offline_all_return).item()
                # wandb_stat["avg_test_np_online_all_return"] = np.mean(avg_test_np_online_all_return).item()
                # wandb_stat["avg_train_online_final_return"] = np.mean(avg_train_online_final_return).item()
                # wandb_stat["avg_train_offline_final_return"] = np.mean(avg_train_offline_final_return).item()
                # wandb_stat["avg_train_np_online_final_return"] = np.mean(avg_train_np_online_final_return).item()
                # wandb_stat["avg_train_online_all_return"] = np.mean(avg_train_online_all_return).item()
                # wandb_stat["avg_train_offline_all_return"] = np.mean(avg_train_offline_all_return).item()
                # wandb_stat["avg_train_np_online_all_return"] = np.mean(avg_train_np_online_all_return).item()
                #
                # wandb_stat["avg_test_online_all_return_one_shot"] = np.mean(avg_test_online_all_return_one_shot).item()
                # wandb_stat["avg_test_offline_all_return_one_shot"] = np.mean(avg_test_offline_all_return_one_shot).item()
                # wandb_stat["avg_test_np_online_all_return_one_shot"] = np.mean(avg_test_np_online_all_return_one_shot).item()
                #
                # wandb_stat["avg_train_online_all_return_one_shot"] = np.mean(avg_train_online_all_return_one_shot).item()
                # wandb_stat["avg_train_offline_all_return_one_shot"] = np.mean(avg_train_offline_all_return_one_shot).item()
                # wandb_stat["avg_train_np_online_all_return_one_shot"] = np.mean(avg_train_np_online_all_return_one_shot).item()
    ...
    def evaluate(self, epoch):
        ...
        test_online_final_returns, test_online_all_returns, test_offline_final_returns, \
        test_offline_all_returns, test_np_online_final_returns, test_np_online_all_returns, \
        test_online_all_returns_one_shot, test_offline_all_returns_one_shot, test_np_online_all_returns_one_shot = self._do_eval(self.eval_tasks, epoch, buffer=self.eval_buffer)
        ...
        avg_test_online_all_return_one_shot = np.mean(np.stack(test_online_all_returns_one_shot), axis=0)
        avg_test_offline_all_return_one_shot = np.mean(np.stack(test_offline_all_returns_one_shot), axis=0)
        avg_test_np_online_all_return_one_shot = np.mean(np.stack(test_np_online_all_returns_one_shot), axis=0)
        ...
        train_online_final_returns, train_online_all_returns, train_offline_final_returns, \
        train_offline_all_returns, train_np_online_final_returns, train_np_online_all_returns, \
        train_online_all_returns_one_shot, train_offline_all_returns_one_shot, train_np_online_all_returns_one_shot = self._do_eval(indices, epoch, buffer=self.replay_buffer)

        eval_util.dprint('train online all returns')
        eval_util.dprint(train_online_all_returns)
        eval_util.dprint('train offline all returns')
        eval_util.dprint(train_offline_all_returns)
        eval_util.dprint('train np_online all returns')
        eval_util.dprint(train_np_online_all_returns)
        ...
        avg_train_online_all_return_one_shot = np.mean(np.stack(train_online_all_returns_one_shot), axis=0)
        avg_train_offline_all_return_one_shot = np.mean(np.stack(train_offline_all_returns_one_shot), axis=0)
        avg_train_np_online_all_return_one_shot = np.mean(np.stack(train_np_online_all_returns_one_shot), axis=0)
        ...
        return avg_test_online_final_return, avg_test_offline_final_return, avg_test_np_online_final_return, \
               np.mean(avg_test_online_all_return), np.mean(avg_test_offline_all_return), np.mean(avg_test_np_online_all_return), \
               avg_train_online_final_return, avg_train_offline_final_return, avg_train_np_online_final_return, \
               np.mean(avg_train_online_all_return), np.mean(avg_train_offline_all_return), np.mean(avg_train_np_online_all_return), \
               np.mean(avg_test_online_all_return_one_shot), np.mean(avg_test_offline_all_return_one_shot), np.mean(avg_test_np_online_all_return_one_shot), \
               np.mean(avg_train_online_all_return_one_shot), np.mean(avg_train_offline_all_return_one_shot), np.mean(avg_train_np_online_all_return_one_shot)
