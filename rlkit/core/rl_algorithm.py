...
class OfflineMetaRLAlgorithm(metaclass=abc.ABCMeta):
	...
	def init_buffer(self):
		...
		for train_path, train_task_idx in zip(train_paths, train_task_idxs):
			...
			sum_reward_train_lst.append(sum(list(trj_npy[:, 2])))
		...
		for eval_path, eval_task_idx in zip(eval_paths, eval_task_idxs):
			...
			sum_reward_eval_lst.append(sum(list(trj_npy[:, 2])))
	...
	def _try_to_eval(self, epoch):
		...
		if self._can_evaluate():
			train_returns, avg_train_return, avg_test_return = self.evaluate(epoch)
	...
	def train(self):
		...
		csv_file_name = self.csv_name + ".csv"

		with open(csv_file_name, "w", encoding='utf-8', newline='') as f:
			csv_head = ["AverageTrainReturn_all_train_tasks", "AverageReturn_all_train_tasks", "AverageReturn_all_test_tasks"]
			writer = csv.writer(f)
		...
		with open(csv_file_name, "w", encoding='utf-8', newline='') as f:
			...
			for it_ in gt.timed_for(range(self.num_iterations), save_itrs=True):
				...
				for train_step in range(self.num_train_steps_per_itr):
					...
					z_means, z_vars, wandb_stat = self._do_training(indices, zloss=self.is_zloss)
				...
				if self.allow_eval:
					...
					train_returns, avg_train_return, avg_test_return = self._try_to_eval(it_)
