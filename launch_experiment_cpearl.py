...
import wandb
from rlkit.torch.New_networks import SelfAttnEncoder, Ratio, Three_Ratio
...
def experiment(variant, seed=None):
    ...
    context_discriminator = SelfAttnEncoder(
    ...
    if variant['algo_params']["is_zloss"] and not variant['algo_params']["use_information_bottleneck"]:
        print("Since variant[algo_params][is_zloss] and variant[algo_params][use_information_bottleneck], use PEARLAgent." )
        agent = PEARLAgent(
            latent_dim,
            club_input_dim,
            context_encoder,
        ...
        print("Else situation, use OldPEARLAgent.")
        agent = OldPEARLAgent(
            latent_dim,
            context_encoder,
            club_model,
    ...
    deep_metric_context = FlattenMlp(hidden_sizes=[50, 50],
                             input_size=context_encoder_output_dim * 2,
                             output_size=1, )
        ...
        if 1:  # foce random
            ...
            if 'goal_radius' in variant['env_params']:
                algorithm = CPEARL(
                    ...
                    # nets=[agent, qf1, qf2, vf, c, rew_decoder, transition_decoder, task_id_decoder, deep_metric_context, Bisimulation_ratio, InfoNCE_ratio, Context_ratio],
